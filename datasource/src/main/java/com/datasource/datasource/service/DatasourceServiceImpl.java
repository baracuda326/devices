...
public class DatasourceServiceImpl implements DatasourceService {
    ...
    @Autowired
    public DatasourceServiceImpl(DeviceFeignRepository deviceFeignClient, DeviceRepository deviceRepository) {
        ...
        while (flag) {
            for (Device device : list) {
                if (count >= 20 && device.getId() == 6) {
                    device.setAirTemperature(0);
                    device.setWaterTemperature(0);
                    device.setPressure(0);
                    device.setWindSpeed(0);
                    device.setDateTime(LocalDateTime.now().format(dateTimeFormatter));
                    device.setIsAlive(0);
                }
                sendData(device);
                count++;
    ...
    public void sendData(Device dto) {
    ...
    private List<Device> mapper(List<DeviceEntity> list) {
        ...
        for (DeviceEntity deviceEntity : list) {
            devices.add(Device.builder()
                    ...
                    .windSpeed(deviceEntity.getWindSpeed())
                    .waveHeight(deviceEntity.getWaveHeight())
                    .waterTemperature(deviceEntity.getWaterTemperature())
                    .airTemperature(deviceEntity.getAirTemperature())
                    .pressure(deviceEntity.getPressure())
                    .isAlive(deviceEntity.getIsAlive())
                    .location(deviceEntity.getLocation())
                    .dateTime(deviceEntity.getDateTime())
                    .build());
        }
        return devices;
    }
